...
config.read("mlflow.cfg")
mlflow_tracking_uri = config.get("mlflow", "mlflow-tracking-uri")
mlflow_experiment_name = config.get("mlflow", "mlflow-experiment-name")
...
def main(args):
    ...
    if args.distributed_rank == 0:
        with mlflow.start_run(run_name="translation-base_en2de") as run:
            d = vars(args)
            d1 = dict(list(d.items())[len(d) // 2 :])
            d2 = dict(list(d.items())[: len(d) // 2])
            mlflow.log_params(d1)
            mlflow.log_params(d2)
    ...
    summary = count_model_parameters(trainer.model)
    ...
    try:
        ...
        if run_id:
            with mlflow.start_run(run_id=run_id):
                mlflow.log_artifact(summary_file, "model_summary")
    finally:
    ...
    if run_id:
        with mlflow.start_run(run_id=run_id):
            mlflow.log_artifacts(args.save_dir, "checkpoints")
...
def train(args, trainer, task, epoch_itr):
    ...
    progress = progress_bar.progress_bar(
        ...
        tensorboard_logdir=(args.tensorboard_logdir if distributed_utils.is_master(args) else None),
...
def validate_and_save(args, trainer, task, epoch_itr, valid_subsets, end_of_epoch):
    ...
    do_validate = (
        ...
        or (
            ...
            and num_updates > 0
            and num_updates % args.validate_interval_updates == i0
        )
